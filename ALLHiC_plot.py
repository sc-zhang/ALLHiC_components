...
def draw_heatmap(read_count_whole_genome_min_size, bin_offset_min_size, ratio, chr_order, min_size):
    ...
    with np.errstate(divide='ignore'):
        hmap = ax.imshow(np.log2(data), interpolation='nearest', origin='lower',cmap=cmap, aspect='equal')
    ...
    for chrn in chr_order:
        ...
        cmap.set_over('black')
        with np.errstate(divide='ignore'):
            hmap = ax.imshow(np.log2(sub_data), interpolation='nearest', origin='lower', cmap=cmap, aspect='equal')
        plt.colorbar(mappable=hmap, cax=None, ax=None, shrink=0.5)
